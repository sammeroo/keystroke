		...
		for(iter = 0;iter < ln;iter++) {
			...
			{	if(strcmp(names[iter], reg->name[p]) == 0)
				{	count[p]++;	total_dist[p] += dist[iter];
					if(count[p] >= 10)	printf("KNN definitive match for: %s\n", reg->name[p]);		// Note: 10 is the acceptance-threshold
				}	}
		}
//////////// FOR DEBUGGING *********
// Print counter
...
printf("no_of_max = %d\n", no_of_max);
		...
		{// In this case we have top 3 matching users. just need to order them by distance
			...
			for(iter = 0; iter < no_of_max; iter++)
				if(topdist[iter] == dmin1)	fprintf(topusers, "1 %s\n", topname[iter]);
			for(iter = 0; iter < no_of_max; iter++)
				if(topdist[iter] == dmin2)	fprintf(topusers, "2 %s\n", topname[iter]);
			for(iter = 0; iter < no_of_max; iter++)
				if(topdist[iter] == dmin3)	fprintf(topusers, "3 %s\n", topname[iter]);
		}
		else if(no_of_max == 2)
...
printf("login_attmpt_usr = %s\n", login_attempt_usr);
	...
	for(i=0; i<l;i++)
	{
		fscanf(topusers, "%s", name);
		if(strcmp(name, login_attempt_usr) == 0)	check = 1;
	}
	rewind(topusers);
